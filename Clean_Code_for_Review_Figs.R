...
#the data
map2 <- read.csv("/Users/lgschaer/Desktop/MURI/PermafrostReview/ord_vars_for_R_V3.csv", row.names = 1) %>%
  filter(Experiment_Lab == 1 & Reactors == 1) %>%
  mutate(across(everything(), ~ifelse(.==0, FALSE,TRUE))) %>%
  rownames_to_column(var = "Article") %>%
  filter(Article != "Nadelhoffer 1991") %>%
  #select(-c("Location_Alaska", #"Location_EasternEurope", "Location_Scandinavia", 
  #         "Location_Canada", "Location_Greenland")) 
  select(c("Article", "Site_PeatMoss","Site_Shrubs","Site_BlackSpruce", "Sequenced_MetaT", "Other_ITS", "Other_MicrobialBiomass", "Other_SIP", 
...
newColNames <- c(
  "Article", "Peat","Shrubs","Black Spruce", "Metatranscriptomic Sequencing", "ITS qPCR", "Microbial Biomass", "Stable Isotope Probing", "16S rRNA Sequencing", "Bacterial qPCR",  "Viral", "Metagenomic Sequencing", "Fungal Sequencing",  
...
#make new df
names_locations <- read.csv("/Users/lgschaer/Desktop/MURI/PermafrostReview/ord_vars_for_R_V3.csv", row.names = 1) %>%
  filter(Experiment_Lab == 1 & Reactors == 1) %>%
  rownames_to_column(var = "Article") %>%
  filter(Article != "Nadelhoffer 1991") %>%
...
order <- c(
  "Article", 
  "Fungal Sequencing", "Viral", "Metatranscriptomic Sequencing", "16S rRNA Sequencing", "Stable Isotope Probing", "ITS qPCR", "Metagenomic Sequencing", "Microbial Biomass",  "Bacterial qPCR",    
  "< 30 Days", "30 - 100 Days", "> 100 Days",
  "Nitrous Oxide", "Volatile Organic Carbon", "Methane","Carbon Dioxide", 
  "< 3 C", "3 - 5 C", "5 - 10 C", "> 10 C",
  "Ramp Up", "Single Temperature", "Compare 2+ Temperatures",
...
upset <- upset(
  ...
  annotations = list(
    "Names"=(
      ggplot(mapping = aes(y = Row))
      +geom_point(aes(fill = Location), shape = 21, size = 6, color = "black")
      +geom_text(label = map3$Article, angle = 90, size = 4, hjust = 0.5)
